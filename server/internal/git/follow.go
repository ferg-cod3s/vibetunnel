...
func (fm *FollowMode) getFollowWorktreePath(repoPath string) (string, error) {
	cmd := exec.Command("git", "config", "tunnelforge.followWorktree")
	cmd.Dir = repoPath
...
func (fm *FollowMode) setFollowWorktreePath(repoPath, worktreePath string) error {
	cmd := exec.Command("git", "config", "--local", "tunnelforge.followWorktree", worktreePath)
	cmd.Dir = repoPath
	return cmd.Run()
}
...
func (fm *FollowMode) unsetFollowWorktreePath(repoPath string) error {
	cmd := exec.Command("git", "config", "--local", "--unset", "tunnelforge.followWorktree")
	cmd.Dir = repoPath
	return cmd.Run()
}
...
if command -v tunnelforge >/dev/null 2>&1; then
    tunnelforge git-event --type="post-checkout" --branch="$current_branch" --repo="$(pwd)" &
fi
`
...
if command -v tunnelforge >/dev/null 2>&1; then
    tunnelforge git-event --type="post-commit" --branch="$current_branch" --repo="$(pwd)" &
fi
`
