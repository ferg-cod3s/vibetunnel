...
func TestBufferAggregator_Unsubscribe(t *testing.T) {
	...
	// Verify client was unsubscribed (check snapshot state)
	time.Sleep(100 * time.Millisecond) // Allow processing time
	state := ba.StateSnapshot()
	if cnt := state.SubscriptionsCount["test-session-123"]; cnt != 0 {
		t.Errorf("Expected 0 subscribers after unsubscribe, got %d", cnt)
	}
}
...
func TestBufferAggregator_ClientDisconnection(t *testing.T) {
	...
	// Verify client is registered using snapshot
	state := ba.StateSnapshot()
	if state.ClientCount != 1 {
		t.Errorf("Expected 1 client, got %d", state.ClientCount)
	}
	if cnt := state.SubscriptionsCount["test-session-disconnect"]; cnt != 1 {
		t.Errorf("Expected 1 subscription for session, got %d", cnt)
	}
	...
	// Verify client is unregistered using snapshot
	stateAfter := ba.StateSnapshot()
	if stateAfter.ClientCount != 0 {
		t.Errorf("Expected 0 clients after disconnect, got %d", stateAfter.ClientCount)
	}
	if cnt := stateAfter.SubscriptionsCount["test-session-disconnect"]; cnt != 0 {
		t.Errorf("Expected 0 subscriptions after disconnect, got %d", cnt)
	}
}
