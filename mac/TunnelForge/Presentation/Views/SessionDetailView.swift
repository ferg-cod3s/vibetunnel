...
struct SessionDetailView: View {
                ...
                if let windowInfo {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        if windowSearchAttempted {
                            ...
                            Text(
                                "Could not find a terminal window for this session. The window may have been closed or the session was started outside TunnelForge."
                            )
    ...
    private func updateWindowTitle() {
        ...
        if let pid = session.pid {
            windowTitle = "\(dir) — TunnelForge (PID: \(pid))"
        } else {
            windowTitle = "\(dir) — TunnelForge"
        }
    }
