...
/// Utility to detect and terminate other TunnelForge instances
@MainActor
enum ProcessKiller {
    private static let logger = Logger(subsystem: BundleIdentifiers.loggerSubsystem, category: "ProcessKiller")

    /// Kill all other TunnelForge instances except the current one
    static func killOtherInstances() {
        let currentPID = ProcessInfo.processInfo.processIdentifier
        logger.info("🔍 Current process PID: \(currentPID)")

        // Find all TunnelForge processes
        let tunnelForgeProcesses = findTunnelForgeProcesses()

        // Kill other instances
        var killedCount = 0
        for process in tunnelForgeProcesses where process.pid != currentPID {
            logger.info("🎯 Found other TunnelForge instance: PID \(process.pid) at \(process.path)")
        ...
        if killedCount > 0 {
            logger.info("🧹 Killed \(killedCount) other TunnelForge instance(s)")

            // Give processes time to fully terminate
            Thread.sleep(forTimeInterval: 0.5)
        } else {
            logger.info("✨ No other TunnelForge instances found")
        }
    }

    /// Find all running TunnelForge processes
    private static func findTunnelForgeProcesses() -> [(pid: Int32, path: String)] {
        ...
        for process in allProcesses where process.path.contains("TunnelForge.app/Contents/MacOS/TunnelForge") {
            logger.debug("🎯 Found TunnelForge process: PID \(process.pid) at \(process.path)")
            processes.append(process)
        }

        logger.info("📊 Found \(processes.count) TunnelForge app processes")
        return processes
    }
