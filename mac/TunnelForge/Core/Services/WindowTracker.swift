...
/// - Map TunnelForge sessions to their terminal windows
...
///
/// ### Design Principles:
/// 1. **Only close what we open**: Windows are only closed if TunnelForge opened them
...
///
/// ### User Experience:
/// - Consistent behavior: All TunnelForge-spawned windows close on termination
...
final class WindowTracker {
    ...
    ///
    /// When TunnelForge spawns a terminal session through AppleScript, we mark
    ...
    /// Registers a session that was opened by TunnelForge.
    ...
    /// Closes the terminal window for a specific session if it was opened by TunnelForge.
    ///
    /// This method implements a key feature where terminal windows are automatically closed
    /// when their associated sessions are terminated, but ONLY if TunnelForge opened them.
    /// This prevents the common issue where killing a process leaves empty terminal windows.
    ///
    /// The method checks if:
    /// 1. The session was opened by TunnelForge (exists in `sessionsOpenedByUs`)
    ...
    func closeWindowIfOpenedByUs(for sessionID: String) -> Bool {
        ...
        guard wasOpenedByUs else {
            logger.info("Session \(sessionID) was not opened by TunnelForge, not closing window")
            return false
        }
