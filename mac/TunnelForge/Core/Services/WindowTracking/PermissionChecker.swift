...
final class PermissionChecker {
    ...
    func checkPermissions() -> Bool {
        if !checkPermissionsDirectly() {
            logger.warning("TunnelForge needs accessibility permissions to focus terminal windows")

            let alert = NSAlert()
            alert.messageText = "Accessibility Permissions Required"
            alert.informativeText = """
            TunnelForge needs accessibility permissions to focus terminal windows when you click on sessions.
