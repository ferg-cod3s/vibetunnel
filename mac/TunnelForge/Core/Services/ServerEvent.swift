...
/// Types of server events that can be received from the TunnelForge server.
///
/// `ServerEventType` defines all possible event types that flow through the Server-Sent Events (SSE)
/// connection between the TunnelForge server and the macOS app. Each event type corresponds to
...
/// from the TunnelForge server to the macOS app. Each event carries contextual information
