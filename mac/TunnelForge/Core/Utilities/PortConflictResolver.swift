...
/// including identification details and helpers to determine if it's a TunnelForge-related process.
struct ProcessDetails {
    ...
    /// Check if this is a TunnelForge process
    var isTunnelForge: Bool {
        if let bundleId = bundleIdentifier {
            return bundleId.contains("tunnelforge") || bundleId.contains("TunnelForge")
        }
        if let path {
            return path.contains("TunnelForge")
        }
        return name.contains("TunnelForge")
    }
    ...
    var isManagedServer: Bool {
        ...
        // Node server with TunnelForge in path
        if name.contains("node") && (path?.contains("TunnelForge") ?? false) {
            return true
        }
        // Bun executable (our vibetunnel binary is a Bun executable)
        if name.contains("bun") && (path?.contains("TunnelForge") ?? false) {
            return true
        }
...
/// for TunnelForge-owned processes. Uses system tools like lsof to detect port usage
...
final class PortConflictResolver {
    ...
    /// Kill a process and optionally its parent TunnelForge instance
    ...
    private func findRootProcess(for process: ProcessDetails) async -> ProcessDetails? {
        ...
        while let parentPid = current.parentPid, parentPid > 1, !visited.contains(parentPid) {
            ...
            if let parentInfo = await getProcessInfo(pid: parentPid) {
                // If parent is TunnelForge, it's our root
                if parentInfo.isTunnelForge {
                    return parentInfo
                }
                current = parentInfo
            } else {
    ...
    private func determineAction(for process: ProcessDetails, rootProcess: ProcessDetails?) -> ConflictAction {
        ...
        // If root process is TunnelForge, kill the whole app
        if let root = rootProcess, root.isTunnelForge {
            logger.info("Root process identified as TunnelForge: \(root.name)")
            return .killOurInstance(pid: root.pid, processName: root.name)
        }

        // If the process itself is TunnelForge
        if process.isTunnelForge {
            logger.info("Process identified as TunnelForge: \(process.name)")
            return .killOurInstance(pid: process.pid, processName: process.name)
        }
