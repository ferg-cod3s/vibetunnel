...
///
/// This struct manages the configuration for the TunnelForge web server,
/// including network settings and startup behavior.
struct ServerConfig {
    /// The port number the server listens on.
    ///
    /// Default is 4021 for the Go server (both production and development).
    /// Users can customize this to avoid port conflicts with other services.
    let port: Int

    /// The dashboard access mode.
    ///
    /// Controls who can access the TunnelForge web dashboard:
