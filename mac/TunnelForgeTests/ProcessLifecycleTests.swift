...
struct ProcessLifecycleTests {
    ...
    func basicProcessSpawning() async throws {
        ...
        let result = try await runProcessWithTimeout(
            ...
            arguments: ["Hello from TunnelForge test"],
            timeoutSeconds: 5
        )
    ...
    func shellCommandExecution() async throws {
        // Test shell command execution patterns used in TunnelForge
        Attachment.record("""
    ...
    func networkCommandValidation() async throws {
        // Test network-related commands that TunnelForge might use
        Attachment.record("""
