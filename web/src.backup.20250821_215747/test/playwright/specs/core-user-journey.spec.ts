...
// This test covers the core user journey that VibeTunnel is designed for:
...
test.describe('Core User Journey - Terminal Interaction', () => {
  ...
  test('should complete the full terminal interaction workflow', async ({ page }) => {
    ...
    if (promptAppeared) {
      ...
      await page.keyboard.type('echo "VibeTunnel Core Test"');
      ...
      try {
        await page.waitForFunction(
          () => {
            ...
            return content.includes('VibeTunnel Core Test');
          },
          { timeout: 10000 }
        );
