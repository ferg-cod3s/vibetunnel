...
# TunnelForge Build and Start Script
# This script builds the project and starts it, with fallback to previous build

WORK_DIR="/home/f3rg/Documents/git/tunnelforge/web"
CLI_FILE="$WORK_DIR/dist/tunnelforge-cli"
BACKUP_CLI="$WORK_DIR/dist/tunnelforge-cli.backup"
LOG_FILE="/var/log/tunnelforge-build.log"

cd "$WORK_DIR"

echo "[$(date)] Starting TunnelForge build and start process..." >> "$LOG_FILE"

# Create backup of current working version if it exists
if [ -f "$CLI_FILE" ]; then
    cp "$CLI_FILE" "$BACKUP_CLI"
    echo "[$(date)] Backed up current CLI to tunnelforge-cli.backup" >> "$LOG_FILE"
fi
