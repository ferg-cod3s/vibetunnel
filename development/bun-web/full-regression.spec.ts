 ...
 * Node.js version and Go/Bun version of VibeTunnel
 */
...
class VibeTunnelTestHelper {
  ...
  async waitForAppReady() {
    ...
    await this.page.waitForSelector('vibetunnel-app', { timeout: 10000 });
...
test.describe('VibeTunnel Full Regression Tests', () => {
  let helper: VibeTunnelTestHelper;

  test.beforeEach(async ({ page }) => {
    helper = new VibeTunnelTestHelper(page);
  });
  ...
  test.describe('Go/Bun Server Tests', () => {
    ...
    test('should load application successfully', async ({ page }) => {
      await expect(page.locator('vibetunnel-app')).toBeVisible();
...
test.describe('Performance and Load Tests', () => {
  test('should handle rapid session creation', async ({ page }) => {
    const helper = new VibeTunnelTestHelper(page);
    await helper.navigateToApp(CONFIG.BUN_SERVER);
    await helper.waitForAppReady();
