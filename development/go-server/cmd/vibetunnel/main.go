...
func printHelp() {
	...
	fmt.Println("  vibetunnel unfollow                     Disable Git follow mode")
	fmt.Println("  vibetunnel git-event                    Notify server of Git event")
	fmt.Println("  vibetunnel systemd [action]             Manage systemd service (Linux)")
	fmt.Println("  vibetunnel version                      Show version")
	...
	fmt.Println("  vibetunnel")
	fmt.Println("  vibetunnel fwd abc123 \"ls -la\"")
	fmt.Println("  AUTH_REQUIRED=true vibetunnel")
...
func handleForwardCommand() {
	if len(os.Args) < 4 {
		...
		fmt.Println("Usage: vibetunnel fwd <session-id> <command>")
		os.Exit(1)
	}
...
func handleGitEventCommand() {
	...
	if eventType == "" || repoPath == "" {
		...
		fmt.Println("Usage: vibetunnel git-event --type=<event-type> --repo=<repo-path> [--branch=<branch>]")
		os.Exit(1)
	}
